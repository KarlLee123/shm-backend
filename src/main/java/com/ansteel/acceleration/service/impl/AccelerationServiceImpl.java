...
public class AccelerationServiceImpl implements AccelerationService {
    ...
    private final AccelerationMapper accelerationMapper;

    public AccelerationServiceImpl(AccelerationMapper accelerationMapper) {
        this.accelerationMapper = accelerationMapper;
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void uploadRawData(AccelerationRawUploadDTO dto) {
        ...
        try {
            int rows = accelerationMapper.insertRawData(dto);
            if (rows != 1) {
                throw new BusinessException("原始加速度数据入库失败");
            }
    ...
    @Override
    public AccelerationRawVO getLatestRaw(AccelerationQueryDTO dto) {
        validateLatestQueryDTO(dto);
        String sensorId = dto.getSensorId().trim();
        return accelerationMapper.selectLatestRaw(sensorId);
    }

    @Override
    public List<AccelerationRawVO> getHistoryRaw(AccelerationQueryDTO dto) {
        ...
        return accelerationMapper.selectHistoryRaw(sensorId, startTime, endTime, limit);
    }

    private void validateRawUploadDTO(AccelerationRawUploadDTO dto) {
        if (dto == null) {
        ...
        if (dto.getAccelerationValue() == null) {
            throw new BusinessException("accelerationValue 不能为空");
        }
        if (!hasText(dto.getCollectTime())) {
            throw new BusinessException("collectTime 不能为空");
        }

        validateDecimal(dto.getAccelerationValue(), "accelerationValue");
        normalizeDateTimeString(dto.getCollectTime(), "collectTime");
    }

    private void validateLatestQueryDTO(AccelerationQueryDTO dto) {
        if (dto == null) {
            throw new BusinessException("请求体不能为空");
        }
        if (!hasText(dto.getSensorId())) {
    ...
    private void validateHistoryQueryDTO(AccelerationQueryDTO dto) {
        if (dto == null) {
            throw new BusinessException("请求体不能为空");
        }
        if (!hasText(dto.getSensorId())) {
