...
本模块固定范围为：
- 前端：1 个业务展示页面 `AccelerationMonitorPage`
- 后端：3 个核心接口
  1. `POST /api/sensor/acceleration/upload`
  2. `POST /api/data/acceleration/latest`
  3. `POST /api/data/acceleration/history`
- 数据库：1 张表 `acceleration_data`
...
要求：
- acceleration 模块必须使用 acceleration 模块专属 key；
- 不允许与其他模块共用 key。
...
| accelerationValue | decimal | 是 | 加速度值 |
| collectTime | string | 是 | 格式固定 `yyyy-MM-dd HH:mm:ss` |

## 2.2 AccelerationQueryDTO
...
- `X-API-KEY: <acceleration_key>`

### Request Body
```json
{
  "sensorId": "SENSOR_001",
  "accelerationValue": 12.34,
...
| acceleration_value | decimal(18,6) | 加速度值 |
...
(sensor_id, acceleration_value, collect_time, create_time, deleted)
VALUES (?, ?, ?, NOW(), 0)
```
...
页面名称固定为：`AccelerationMonitorPage`

页面骨架固定为：

AccelerationMonitorPage
├─ QueryBar
├─ LatestCard
├─ TrendChart
└─ HistoryTable
...
> **监测系统-加速度模块V1.0-审查终稿（直采实时版 / 北京时间版）**
