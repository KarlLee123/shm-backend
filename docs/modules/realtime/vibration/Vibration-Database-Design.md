...
```text
当前建表采用“直采即展示、最小闭环优先”的策略。表中 vibration_value、collect_time 等字段用于承载传感器原始数据，支持直接入库与前端展示；本模块当前阶段不引入 Python 计算与人工核验链路。
```
...
```sql
SELECT id, sensor_id, vibration_value, collect_time
FROM vibration_data
WHERE sensor_id = ?
  AND deleted = 0
