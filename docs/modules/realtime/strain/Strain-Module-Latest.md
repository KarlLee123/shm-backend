...
本模块固定范围为：
- 前端：1 个业务展示页面 `StrainMonitorPage`
- 后端：3 个核心接口
  1. `POST /api/sensor/strain/upload`
  2. `POST /api/data/strain/latest`
  3. `POST /api/data/strain/history`
- 数据库：1 张表 `strain_data`
...
(sensor_id, strain_value, collect_time, create_time, deleted)
VALUES (?, ?, ?, NOW(), 0)
```
...
> **监测系统-应变模块V1.0-审查终稿（直采实时版 / 北京时间版）**
