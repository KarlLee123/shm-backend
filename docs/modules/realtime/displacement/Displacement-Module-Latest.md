...
本模块固定范围为：
- 前端：1 个业务展示页面 `DisplacementMonitorPage`
- 后端：3 个核心接口
...
要求：
- displacement 模块必须使用 displacement 模块专属 key；
- 不允许与其他模块共用 key。
...
| displacementValue | decimal | 是 | 位移值 |
...
页面名称固定为：`DisplacementMonitorPage`
...
> **监测系统-位移模块V1.0-审查终稿（直采实时版 / 北京时间版）**
