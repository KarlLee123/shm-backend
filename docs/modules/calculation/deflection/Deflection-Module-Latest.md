...
- 前端：1 个业务展示页面 `DeflectionMonitorPage`
- 后端：5 个核心接口
  1. `POST /api/sensor/deflection/raw/upload`
  2. `POST /api/sensor/deflection/result/upload`
  3. `POST /api/data/deflection/verify`
  4. `POST /api/data/deflection/latest`
  5. `POST /api/data/deflection/history`
- 数据库：1 张表 `deflection_data`
...
当前 Deflection 模块按最小口径固定为：
- `displacementValue`

说明：
- 当前版本按“位移输入 → 计算挠度”收口；
- 若后续需要多测点位移或组合输入，再单独扩展 DTO 与建表。

### 计算结果层

当前 Deflection 模块计算输出字段固定为：
- `calcDeflection`

### 核验结果层

当前 Deflection 模块核验输出字段固定为：
- `verifiedDeflection`
- `verifyStatus`
- `verifyRemark`
...
### 2.2 DeflectionResultUploadDTO

| 字段名 | 类型 | 必填 | 说明 |
| --- | --- | ---: | --- |
| sensorId | string | 是 | 传感器 ID |
| calcDeflection | decimal | 是 | 计算挠度 |
| collectTime | string | 是 | 对应原始采样时间 |
| calcTime | string | 否 | 计算完成时间 |

### 2.3 DeflectionVerifyDTO

| 字段名 | 类型 | 必填 | 说明 |
| --- | --- | ---: | --- |
| sensorId | string | 是 | 传感器 ID |
| action | string | 是 | `PASS` 或 `FAIL` |
| verifiedDeflection | decimal | 否 | `PASS` 时必填 |
| verifyRemark | string | 否 | `FAIL` 时必填 |
| collectTime | string | 是 | 对应原始采样时间 |
| verifyTime | string | 否 | 核验时间 |

### 2.4 DeflectionQueryDTO
...
本文件只服务于 `deflection` 模块，不与 `stress` 共用命名、不混用结果字段。
